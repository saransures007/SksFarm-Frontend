...
import { selectUpdatedItem } from '@/redux/farm/selectors';
...
export default function UpdateItem({ config, UpdateForm }) {
  ...
  const [currentsks, setCurrentsks] = useState(current ?? resetsks);
  ...
  const onSubmit = (fieldsValue) => {
    ...
    dispatch(sks.update({ entity, id, jsonData: dataToUpdate }));
  };
  useEffect(() => {
    if (isSuccess) {
      form.resetFields();
      setSubTotal(0);
      dispatch(sks.resetAction({ actionType: 'update' }));
  ...
  useEffect(() => {
    if (current) {
      console.log("ediy date", current)
      setCurrentsks(current);
      ...
      if (!formData.taxRate) {
        ...
        console.log("ediy date formData", formData)
