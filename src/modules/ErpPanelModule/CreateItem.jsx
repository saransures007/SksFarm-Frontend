...
import { selectCreatedItem } from '@/redux/farm/selectors';
...
export default function CreateItem({ config, CreateForm }) {
  ...
  const handelValuesChange = (changedValues, values) => {
    ...
    if (items) {
      items.map((item) => {
        if (item) {
          if (item.offsksrice && item.quantity) {
            let offerTotal = calculate.multiply(item['quantity'], item['offsksrice']);
  ...
  useEffect(() => {
    if (isSuccess) {
      form.resetFields();
      dispatch(sks.resetAction({ actionType: 'create' }));
      setSubTotal(0);
      setOfferSubTotal(0);
  ...
  const onSubmit = (fieldsValue) => {
    ...
    dispatch(sks.create({ entity, jsonData: fieldsValue }));
  };
