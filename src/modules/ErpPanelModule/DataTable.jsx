...
import { selectListItems } from '@/redux/farm/selectors';
...
export default function DataTable({ config, extra = [] }) {
  ...
  const handleRead = (record) => {
    dispatch(sks.currentItem({ data: record }));
    navigate(`/${entity}/read/${record._id}`);
  };
  const handleEdit = (record) => {
    const data = { ...record };
    dispatch(sks.currentAction({ actionType: 'update', data }));
  ...
  const handleDelete = (record) => {
    dispatch(sks.currentAction({ actionType: 'delete', data: record }));
    modal.open();
  };
  ...
  const handelDataTableLoad = (pagination) => {
    ...
    dispatch(sks.list({ entity, options }));
  };

  const dispatcher = () => {
    dispatch(sks.list({ entity }));
  };
